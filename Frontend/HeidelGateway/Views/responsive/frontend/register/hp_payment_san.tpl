{block name="hp_payment_san"}
	<div class="{$grid} {$classname}" style='background-color: transparent;'>
		...
		<div class="newreg_{$pm}" id="payType" style="width: 30rem;">
			...
			{if isset($birthdate)}
				<!--{assign var=payment_data value=$regData.$pm.payment_data|json_decode:1}-->
				<!--{html_select_date|utf8_encode time=$regData.birthdate.formatted start_year='-18' end_year='-100' reverse_years='true' day_value_format='%02d' field_order='DMY'}-->
			...
			{if isset($optin)}
				<div>
					<p><input type="checkbox" id="hgw_cb_san" name="CUSTOMER.OPTIN" value="TRUE" class="checkbox"> {$optin}</p>
					<label for="hgw_privacyPolicy">{s name='hp_sanPrivacyPolicy' namespace='frontend/register/hp_payment'}{/s}:</label>
					<div id="hgw_privacyPolicy" style="height:160px;width:30 rem;overflow:auto;">{$privacy_policy}</div>
				</div>
			{/if}		

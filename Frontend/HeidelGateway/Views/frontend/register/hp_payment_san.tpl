	...
	<div class="{$grid} {$classname}" style='background-color: transparent; border-color: transparent;'>
		...
		<div class="newreg_{$pm}" id="payType" style="width: 44rem;">
			...
			{if isset($birthdate_san)}
                ...
                {html_select_date|utf8_encode prefix='DateSan_' time=$payment_data start_year='-10' end_year='-100' reverse_years='true' day_value_format='%02d' field_order='DMY'}
            {else}
				{html_select_date|utf8_encode
				prefix='DateSan_'
				time=$payment_data start_year='-14' end_year='-100' reverse_years='true'

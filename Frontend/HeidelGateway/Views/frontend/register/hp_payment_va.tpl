	...
	<div class="{$grid} {$classname}" style='background-color: transparent; border: 0;'>
		{if isset($regData.$pm)}
			...
			<div class="reuse_{$pm}" {if {$hasReg{$pm}}} style="display: block;" {/if}>
				...
				<table>
					<tr><td>{s name='hp_mail' namespace='frontend/register/hp_payment'}{/s}:</td><td>{$regData.$pm.email}</td></tr>
				</table>
			</div>
		{/if}
		...
		<div class="newreg_{$pm}" {if {$hasReg{$pm}}} style="display: none;" {/if}>
			...
			<label>{s name='hp_mail' namespace='frontend/register/hp_payment'}{/s}*:</label>
